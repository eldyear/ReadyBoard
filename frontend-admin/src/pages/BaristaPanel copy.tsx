...
export default function BaristaPanel({ boardId, boardName }: Props) {
    ...
    useEffect(() => {
        ...
        fetch(`${API}/orders?board_id=${boardId}`, {
    ...
    useEffect(() => {
        ...
        const connect = () => {
            ...
            const url = WS_BASE.startsWith('/')
                ? `${proto}://${location.host}${WS_BASE}?board_id=${boardId}`
                : `${WS_BASE}?board_id=${boardId}`
            setWsStatus('connecting')
            ws = new WebSocket(url)
            ws.onopen = () => { setWsStatus('connected'); backoff = 1000 }
            ws.onmessage = e => {
                try {
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
                <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: '1.5rem', borderBottom: '1px solid rgba(0,0,0,0.1)', marginBottom: '2rem' }}>
                    <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                        ...
                        <div>
                            <h1 style={{ fontSize: '2.5rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '-0.05em', margin: 0 }}>Terminal {boardName || ''}</h1>
